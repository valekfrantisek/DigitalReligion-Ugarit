...
ROOT_PATH = os.getcwd()
UGARIT_TEXTS_DATABASE_PATH = os.path.join(ROOT_PATH, '01_CURRENT_DATABASE_FILES', 'CSV', 'UGARIT_TEXTS_DATABASE.csv')
STATS_PATH = os.path.join(ROOT_PATH, '02_STATISTICAL_DATA')
...
# NOTE: when using function that consider language of SAU first, there are some exceptions that need reclassification.
language_exceptions_KTU_detailed = {
    '1.110': 'Ugaritic;Hurrian',
    '1.111': 'Ugaritic;Hurrian',
    '1.116': 'Ugaritic;Hurrian',
    '1.33': 'Hurrian',
    '1.35': 'Hurrian',
    '1.36': 'Hurrian',
    '1.26': 'Hurrian',
...
language_exceptions_KTU_multilingual = {
    '1.110': 'multilingual',
    '1.111': 'multilingual',
    '1.116': 'multilingual',
    '1.33': 'Hurrian',
...
def get_all_lang_combinations(input_db):
    ...
    for row_id in db.index:
        languages = normalise_lang(db.iloc[row_id]['Language'], use_multilingual=False)
            
        all_lang_combinations.append(languages)
            
        languages = normalise_lang(db.iloc[row_id]['SAU Language'], use_multilingual=False)
...
def list_languages_sau_et_rsti(input_db, outfile_name:str, use_multilingual=True, ignore_langs=['Latin ("RSTI")', 'Phoenician ("RSTI")', 'Latin (SAU)', 'Phoenician (SAU)']):
    ...
    for row_id in input_db.index:
        languages_rsti = normalise_lang(input_db.iloc[row_id]['Language'], use_multilingual=use_multilingual)
        languages_rsti = f'{languages_rsti} ("RSTI")'

        sau_languages = normalise_lang(input_db.iloc[row_id]['SAU Language'], use_multilingual=use_multilingual)
...
def list_locations_et_languages(input_db:pd.core.frame.DataFrame, lang_by:str, outfile_name: str, ignore_langs=['Latin', 'Phoenician'], use_multilingual=True):
    """
    This function counts languages that are associated with individual archives.

    Args:
        input_db (pd.core.frame.DataFrame): pandas dataframe from which the data are extracted
        lang_by (str): select "rsti" or "sau" (does it count language data from data obtained from RSTI or from SAU?)
    ...
    for row_id in input_db.index:
        general_area = input_db.iloc[row_id]['Archive/General area']
        if type(general_area) == float:
            general_area = input_db.iloc[row_id]['SAU Archive/General area']
            if type(general_area) == float:
                general_area = 'Other/unknown'

        if lang_by == 'rsti':
            languages = normalise_lang(input_db.iloc[row_id]['Language'], use_multilingual=use_multilingual)
        elif lang_by == 'sau':
            languages = normalise_lang(input_db.iloc[row_id]['SAU Language'], use_multilingual=use_multilingual)
        else:
            print('WARNING: there is error in data selection setting, choose "rsti" or "sau" for lang_by parameter.')
            return
...
def list_locations_et_languages_first_sau(input_db:pd.core.frame.DataFrame, outfile_name: str, ignore_langs=['Latin', 'Phoenician'], use_multilingual=True):
    """
    ...
    for row_id in input_db.index:
        languages = normalise_lang(input_db.iloc[row_id]['SAU Language'], use_multilingual=use_multilingual)
        if languages == 'unknown/unassigned':
            languages = normalise_lang(input_db.iloc[row_id]['Language'], use_multilingual=use_multilingual)
            
        if use_multilingual:
            if input_db.iloc[row_id]['KTU3'] in language_exceptions_KTU_multilingual:
                languages = language_exceptions_KTU_multilingual[input_db.iloc[row_id]['KTU3']]
        else:
            if input_db.iloc[row_id]['KTU3'] in language_exceptions_KTU_detailed:
                languages = language_exceptions_KTU_detailed[input_db.iloc[row_id]['KTU3']]
...
def compare_languages_in_RSTI_et_SAU(input_db:pd.core.frame.DataFrame, outfile_name='SAU_RSTI_lang_flow.csv', use_multilingual=False, ignore_langs=None):
    ...
    for row_id in input_db.index:
        SAU_lang = normalise_lang(input_db.iloc[row_id]['SAU Language'], use_multilingual=use_multilingual)
        RSTI_lang = normalise_lang(input_db.iloc[row_id]['Language'], use_multilingual=use_multilingual)
...
def list_languages(input_db, lang_by:str, outfile_name:str, use_multilingual=True, ignore_langs=['Latin', 'Phoenician'], only_multi=False):
    languages_counts = defaultdict(int)
...
def list_languages_sau_first(input_db, outfile_name:str, use_multilingual=True, ignore_langs=['Latin', 'Phoenician'], only_multi=False, only_Ugarit_MeB_RIH=True):
    languages_counts = defaultdict(int)

    for row_id in input_db.index:
        if only_Ugarit_MeB_RIH:
            if type(input_db.iloc[row_id]['Excavation Siglum']) == float:
                continue
...
def list_locations_et_ktu_genres(input_db, outfile_name:str):
    ...
    for row_id in input_db.index:
        ...
        ktu_num = str(input_db.iloc[row_id]['KTU3'])
...
def get_religious_texts_stats(input_db, outfile_name:str, detailed_relation=False):
    ...
    for row_id in input_db.index:
        ...
        related_texts = input_db.iloc[row_id]['"Clemens 2001"']
        ...
        possible_relevant_genre = input_db.iloc[row_id]['possible relevant (religious) genres']
...
def get_religious_texts_detailed_stats(input_db, outfile_name:str):
    ...
    for row_id in input_db.index:
        ...
        genre_according_to_sau = input_db.iloc[row_id]['SAU genre']
...
def religious_languages(input_db, outfile_name:str, lang_by:str, use_multilingual=True, ignore_langs=['Latin', 'Phoenician']):
...
def religious_languages_sau_first(input_db, outfile_name:str, use_multilingual=True, ignore_langs=['Latin', 'Phoenician']):
    locations_languages = defaultdict(dict)
...
def list_genres_detailes_in_cluster(input_db, outfile_name_prefix:str, cluster='Royal Palace'):
    ...
    for row_id in input_db.index:
        ...
        if general_area == cluster:
            detail_in_cluster = input_db.iloc[row_id]['Detail in General']
            if type(detail_in_cluster) == float:
                detail_in_cluster = input_db.iloc[row_id]['SAU Detail in General']
...
if __name__ == "__main__":
    ...
    list_locations_et_languages(input_db=db, lang_by='rsti', outfile_name='RSTI_locations_et_languages_merge_multilingual.csv', use_multilingual=True)
    list_locations_et_languages(input_db=db, lang_by='sau', outfile_name='SAU_locations_et_languages_merge_multilingual.csv', use_multilingual=True)
    list_locations_et_languages_first_sau(input_db=db, outfile_name='locations_et_languages_SAU_first.csv')

    list_locations_et_languages(input_db=db, lang_by='rsti', outfile_name='RSTI_locations_et_languages_full.csv', use_multilingual=True)
    list_locations_et_languages(input_db=db, lang_by='sau', outfile_name='SAU_locations_et_languages_full.csv', use_multilingual=True)
    ...
    list_languages_sau_et_rsti(input_db=db, outfile_name='languages_SAU_et_RSTI.csv')
    
    """ General language statistics """
    print('Doing general language statistics...')
    list_languages(input_db=db, lang_by='rsti', outfile_name='RSTI_languages_stats.csv')
    list_languages(input_db=db, lang_by='rsti', outfile_name='RSTI_languages_stats_multi.csv', only_multi=True, use_multilingual=False)

    list_languages(input_db=db, lang_by='sau', outfile_name='SAU_languages_stats.csv')
    list_languages(input_db=db, lang_by='sau', outfile_name='SAU_languages_stats_multi.csv', only_multi=True, use_multilingual=False)
    
    list_languages_sau_first(input_db=db, outfile_name='language_statistics_sau_first.csv')
    list_languages_sau_first(input_db=db, outfile_name='language_statistics_sau_first_multi.csv', only_multi=True, use_multilingual=False)
    ...
    religious_languages(input_db=db, outfile_name='RSTI_religious_langs_stats.csv', lang_by='rsti')
    religious_languages(input_db=db, outfile_name='SAU_religious_langs_stats.csv', lang_by='sau')
    religious_languages_sau_first(input_db=db, outfile_name='religious_langs_stats_SAU_first.csv')
